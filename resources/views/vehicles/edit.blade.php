                    ...
                    <form method="post" action="{{route('inout.store')}}">
                        ...
                        <div class="form-row justify-content-center">
                            <div class="col-md-10 mb-3">
                              ...
                              <input type="date" class="form-control" value="{{$item->date}}" format="Y-m-d H:i:s" name="date" required>
                            </div>
                            <div class="col-md-10 mb-3">
                              <label for="validationDefault01">Tipo</label>
                              <select name="type" class="form-control">
                                <option value="in" {{($item->type=='in'?'selected':'')}}>Entrada</option>
                                <option value="out" {{($item->type=='out'?'selected':'')}}>Salida</option>
                              </select>
                            </div>
                            ...
                            <div class="col-md-5 mb-3">
                              ...
                              <select class="form-control" name="vehicle_id" required>
                                @foreach(\App\Vehicle::cursor() as $vh)
                                  <option value="{{$vh->id}}" {{($item->vehicle_id==$vh->id?'selected':'')}}>{{$vh->plate}}</option>
                                @endforeach
                              </select>
                            </div>
                            <div class="col-10">
